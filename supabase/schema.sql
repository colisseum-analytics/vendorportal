...
-- Also callable by the service role — the scheduled inactive-user cleanup
-- job (see 20260912130000_inactive_user_cleanup.sql) runs with no user
-- session at all, only the service role, same as export_platform_backup().
...
--   - "Signed in, no activity": warned by email once inactive for 13
--     days, then deleted at 20 days if they haven't signed back in
--     since. Driven by a daily GitHub Actions job (see
--     .github/workflows/inactive-user-cleanup.yml), not pg_cron.
...
-- email, from the admin Residents page. Only works for accounts that
-- already exist (no deferred/pending-invite mechanism, unlike
-- admin_invites for co-admins) — by design, to keep this simple.
-- SECURITY DEFINER because looking up a user by email requires reading
-- auth.users, which neighborhood admins can't do directly, and
-- members_self_insert only allows inserting your own row.
