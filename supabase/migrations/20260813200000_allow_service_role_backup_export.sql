...
-- access (it bypasses RLS entirely), so also allowing requests running as
-- the `service_role` Postgres role doesn't grant anything new — it just
-- lets this convenience RPC work for that already-trusted caller too.
